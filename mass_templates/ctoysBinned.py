...
import os
import sys
gSystem.CompileMacro("binnedToys.C", "gOck")
gSystem.Load("binnedToys_C")
...
def main():
    ...
    parser.add_argument("-b", "--nbins", default = 100, help="number of bins", type=int)
    parser.add_argument("-t", "--ntoys", default = 10, help="number of toys to generate", type=int)
    parser.add_argument("-min", default = 25.0, help = "bin min", type=float)
    parser.add_argument("-max", default = 225.0, help = "bin max", type=float)
    ...
    if args.nbins <= 0:
        print "Number of bins must be > 0. Defaulting to 100"
        args.nbins = 100
    ...
    moments = createToys(t, obsName, args.cut, args.ntoys, args.nbins, args.min, args.max)
